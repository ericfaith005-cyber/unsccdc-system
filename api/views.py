...
# --- 🛰️ 1. THE IMPERIAL WORKERS (HELPER BUTTONS) ---

def download_button(obj):
    return mark_safe(f'<a href="/api/download-report/{obj.account_number}/" target="_blank" style="background-color: #D4AF37; color: black; padding: 6px 12px; border-radius: 8px; font-weight: bold; text-decoration: none; border: 1px solid #000;">📥 DOWNLOAD PDF</a>')

def payslip_button(obj):
    return mark_safe(f'<a href="/api/download-payslip/{obj.id}/" target="_blank" style="background-color: #002366; color: white; padding: 5px 10px; border-radius: 5px; font-weight: bold; text-decoration: none;">📄 PRINT SLIP</a>')

# --- 🏢 2. THE INSTITUTIONAL REGISTRY (SCHOOLS) ---

@admin.register(School)
class SchoolAdmin(admin.ModelAdmin):
    list_display = ('name', 'school_code', 'district', 'is_verified')
    fieldsets = (
        ('CORE IDENTITY', {'fields': ('name', 'director', 'address', 'district', 'school_motto', 'uneb_center_number')}),
        ('GATEWAY CREDENTIALS', {'fields': ('school_code', 'api_password', 'total_revenue_collected')}),
    )

# --- 👨‍🎓 3. THE GRID-BASED ACADEMIC HUB ---
...
# --- 💰 4. THE NATIONAL SCHOOLPAY LEDGER (BANK LEVEL) ---
...
class SchoolPayLedgerAdmin(admin.ModelAdmin):
    ...
    list_display = ('txn_id', 'student_name', 'acc_number', 'amount_paid', 'balance_remaining', 'channel_badge', 'timestamp')
    list_filter = ('school', 'timestamp')
    search_fields = ('receipt_number', 'student__full_name', 'student__payment_code')

    # 🛡️ THE AUDIT LOCK (UNEDITABLE)
    ...
    def changelist_view(self, request, extra_context=None):
        now = timezone.now()
        txs = self.get_queryset(request)
        extra_context = extra_context or {}
        
        extra_context['d_total'] = txs.filter(timestamp__date=now.date()).aggregate(Sum('amount'))['amount__sum'] or 0
        extra_context['w_total'] = txs.filter(timestamp__date__gte=now.date()-timedelta(days=7)).aggregate(Sum('amount'))['amount__sum'] or 0
        extra_context['m_total'] = txs.filter(timestamp__date__gte=now.date()-timedelta(days=30)).aggregate(Sum('amount'))['amount__sum'] or 0
        
        return super().changelist_view(request, extra_context=extra_context)

    def txn_id(self, obj): return mark_safe(f'<code style="color:#D4AF37;">{obj.receipt_number}</code>')
    def student_name(self, obj): return obj.student.full_name.upper()
    def acc_number(self, obj): return obj.student.payment_code
    def amount_paid(self, obj): return mark_safe(f'<b style="color:#00ff00;">+ {obj.amount:,.0f} UGX</b>')
    def channel_badge(self, obj):
        ch = obj.raw_data.get('sourceChannel', 'BANK').upper()
        color = "#FCDC04" if "MTN" in ch else "#D90000"
        return mark_safe(f'<span style="background:{color}; color:#000; padding:2px 8px; border-radius:4px; font-size:9px; font-weight:bold;">{ch}</span>')
    ...
    def balance_remaining(self, obj):
        try:
            ...
            return mark_safe(f'<b style="color:{"#D90000" if bal > 0 else "#00ff00"}">{bal:,.0f} UGX</b>')
        except: return "0"

# --- 👔 5. REMAINING NATIONAL PILLARS ---
@admin.register(Staff)
class StaffAdmin(admin.ModelAdmin):
    list_display = ('full_name', 'designation', 'school')

admin.site.register([Parent, Student, FeesTracker, Subject, StaffPayroll, FinancialCommandCenter, NationalTopPerformer, SubjectAssignment])
