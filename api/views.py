...
from .models import Student, Parent, School, DataIngestionVault, FeesTracker
    ...
    try:
        with pdfplumber.open(vault.uploaded_pdf.path) as pdf:
            for page in pdf.pages:
                ...
                # 🧠 THE Hub Hub Hub AUTO-MAPPING BRAIN
                # We identify which column is which by looking at the first row (Header)
                header = [str(h).upper() if h else "" for h in table[0]]
                
                # Find indices based on keywords
                ...
                idx_class = next((i for i, h in enumerate(header) if "CLASS" in h or "LEVEL" in h), 2)
                idx_stream = next((i for i, h in enumerate(header) if "STREAM" in h), 3)
                idx_parent = next((i for i, h in enumerate(header) if "PARENT" in h or "GUARDIAN" in h), 4)
                idx_phone = next((i for i, h in enumerate(header) if "PHONE" in h or "CONTACT" in h), 5)
                ...
                for row in table[1:]: # Skip the header
                    ...
                    with transaction.atomic():
                        # 1. ORGANIZE PARENT
                        p_phone = str(row[idx_phone]).strip() if row[idx_phone] else "000000000"
                        parent_obj, _ = Parent.objects.get_or_create(
                            phone_number=p_phone,
                            defaults={
                                'full_name': str(row[idx_parent]).strip() or "National Guardian",
                                'secure_pin': '123456'
                            }
                        )
                        ...
                        # 3. INITIALIZE TREASURY (Auto-set 0 balance for new students)
                        FeesTracker.objects.get_or_create(student=student_obj)
                        
                        count += 1
                        status = "NEW" if created else "UPDATED"
                        summary.append(f"{student_obj.full_name} ({status})")

        vault.processed = True
        vault.total_records = count
        vault.save()
        
        # 🎨 THE Hub Hub Hub PRESTIGE SUCCESS SCREEN
        html_summary = "".join([f"<li>{s}</li>" for s in summary[:10]]) # Show first 10
        return HttpResponse(f"""
            <body style="background:#050505; color:#fff; font-family:sans-serif; text-align:center; padding:50px;">
                <div style="max-width:600px; margin:auto; border:2px solid #D4AF37; padding:30px; border-radius:20px; background:rgba(212,175,55,0.02);">
                    <h1 style="color:#D4AF37;">⚡ INGESTION COMPLETE</h1>
                    <p style="font-size:18px;">Successfully Arranged <b>{count}</b> Students</p>
                    <hr style="border-color:#222;">
                    <ul style="text-align:left; color:#888; font-size:12px;">{html_summary} ...</ul>
                    <a href="/admin/api/dataingestionvault/" style="display:inline-block; margin-top:20px; color:#000; background:#D4AF37; padding:10px 20px; border-radius:10px; text-decoration:none; font-weight:900;">RETURN TO OFFICE</a>
                </div>
            </body>
        """)
