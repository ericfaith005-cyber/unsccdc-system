...
def draw_keb_slip_layout(p, student, school, y_offset):
    ...
    rank_bg = colors.HexColor("#004d40") # Deep Sovereign Teal for Ranking
    ...
    # 2. 🌌 KEB LOGO WATERMARK (CENTERED & FAINT)
    if school.keb_logo and os.path.exists(school.keb_logo.path):
        p.saveState()
        p.setFillAlpha(0.04) # 💎 Make it very faint
        p.drawImage(school.keb_logo.path, width/2 - 100, base_y - 300, width=200, height=200, mask='auto')
        p.restoreState()

    # 3. 🏛️ NATIONAL TOP HEADERS
    p.setFillColor(colors.black); p.setFont("Times-Bold", 9)
    ...
    # 🏢 SCHOOL IDENTITY (IMMEDIATELY AFTER KEB)
    p.setStrokeColor(rich_gold); p.setLineWidth(0.5)
    p.line(45, base_y - 50, width - 45, base_y - 50)
    
    lx, ly = 45, base_y - 120
    if school.logo and os.path.exists(school.logo.path):
        p.drawImage(school.logo.path, lx, ly, width=60, height=60, mask='auto')
    
    p.setFillColor(gov_blue); p.setFont("Times-Bold", 13)
    p.drawString(lx + 70, base_y - 75, school.name.upper())
    p.setFillColor(colors.black); p.setFont("Times-Bold", 8)
    p.drawString(lx + 70, base_y - 88, f"TEL: {getattr(school, 'phone', '---')} | {getattr(school, 'phone2', '')}")
    p.drawString(lx + 70, base_y - 100, f"EMAIL: {getattr(school, 'email', '---')}")
    p.setFont("Times-Italic", 7.5); p.setFillColor(colors.grey)
    p.drawString(lx + 70, base_y - 112, f"MOTTO: \"{getattr(school, 'school_motto', 'Excellence')}\"")

    # 4. 📸 STUDENT BIOMETRICS (TOP RIGHT)
    px, py, pw, ph = width - 120, base_y - 130, 75, 90
    if student.photo and os.path.exists(student.photo.path):
    ...
    p.setFillColor(colors.black); p.setFont("Times-Bold", 9)
    sx = width - 300 
    p.drawString(sx, base_y - 75, f"NAME: {student.full_name.upper()}")
    p.drawString(sx, base_y - 90, f"LEVEL: {student.current_class}")
    p.drawString(sx, base_y - 105, f"YEAR: 2026")
    p.drawString(sx, base_y - 120, f"REGISTRY: VERIFIED")

    # 🧮 5. SOVEREIGN RANKING LOGIC (Result 1 unless all E)
    ...
    # 💎 THE "RESULT 1" LAW
    # Check if every single grade is 'E' or lower (fails)
    all_fails = all(r.score < 50 for r in results_qs) if results_qs.exists() else True
    
    p.setFillColor(rank_bg); p.rect(45, base_y - 155, 260, 22, fill=1, stroke=0)
    ...
    p.drawString(55, base_y - 148, rank_text)

    # 6. 📊 THE MARKS TABLE (TEACHER REMOVED)
    headers = ['SUBJECT NAME', 'SCORE (%)', 'GRADE', 'INTERPRETATION']
    col_widths = [180, 80, 80, 170] # Total 510 - wider for better look
    data_rows = [headers]

    for r in results_qs:
        score = r.score
        # 💎 10-POINT BRACKET INTERPRETATIONS
        if score >= 90: interp = "EXCEPTIONAL MASTERY"
        elif score >= 80: interp = "ADVANCED COMPETENCY"
        elif score >= 70: interp = "STRONG PROFICIENCY"
        elif score >= 60: interp = "SATISFACTORY PROGRESS"
        elif score >= 50: interp = "BASIC ACHIEVEMENT"
        elif score >= 40: interp = "ELEMENTARY LEVEL"
    ...
    table.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), gov_blue), ('TEXTCOLOR', (0,0), (-1,0), colors.white),
        ('FONTNAME', (0,0), (-1,-1), 'Times-Bold'), ('FONTSIZE', (0,0), (-1,-1), 8.5),
        ('GRID', (0,0), (-1,-1), 0.1, colors.black), ('ALIGN', (0,0), (-1,-1), 'CENTER'),
        ('ROWBACKGROUNDS', (0,1), (-1,-1), [colors.white, colors.whitesmoke]),
    ]))
    table.wrapOn(p, width, height)
    table.drawOn(p, 45, base_y - 300)

    # 7. 📖 DYNAMIC GRADE DESCRIPTOR (LEVEL SENSITIVE)
    p.setFillColor(colors.black); p.setFont("Times-Bold", 8)
    p.drawString(45, base_y - 315, "NATIONAL EVALUATION SUMMARY:")
    
    if is_a_level:
        desc_text = f"UACE STANDARD: Candidate scored {total_uace_points} points. 12+ qualifies for direct University entry."
    else:
        desc_text = "UCE STANDARD: Evaluated on New Competency Curriculum. Grades A-E represent mastery levels."
    
    p.setFont("Times-Roman", 8)
    p.drawString(45, base_y - 328, desc_text)

    # 8. ✍️ FOOTER: SIGNATURE & KEB LOGO SEAL
    p.setStrokeColor(gov_blue); p.line(45, base_y - 375, 200, base_y - 375)
    p.setFillColor(colors.black); p.setFont("Times-Bold", 8)
    p.drawString(45, base_y - 388, "EXAMINATIONS SECRETARY (KEB)")
    
    # 🛡️ THE KEB LOGO IN THE SEAL (BOTTOM RIGHT)
    seal_x, seal_y = width - 90, base_y - 395
    if school.keb_logo and os.path.exists(school.keb_logo.path):
